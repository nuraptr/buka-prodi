...
if(isset($_POST['add'])){
  ...
  $q =mysqli_query($dbconnect,"SELECT nip FROM dosen WHERE nip='$nip_user'");
  $rowcount=mysqli_num_rows($q);

  if ($rowcount == 0){
    header("location:register.php?msg=failed");
  }else{
    // $query="INSERT INTO user VALUES ('$nip_user', '$email', '".md5($password)."')";
     // $query="INSERT INTO user VALUES ('$nip_user', '$email', md5('$password'), 'pengusul')";
    $cekUser =mysqli_query($dbconnect,"SELECT nip_user FROM user WHERE nip_user='$nip_user'");
    $rCount = mysqli_num_rows($cekUser);

    if ($rCount > 0){
      header("location:register.php?msg=gagal");
    }else{
      $query= "INSERT INTO user VALUES (NULL, '$nip_user', '$email', MD5('$password'), 'pengusul')";
        ...
        <div class="page-content col-md-7">
          <div style="margin: 100px;" class="col-md-9 col-md-offset-2">
            <div class="pull-right">
              <a class="btn btn-circle btn-success" href="panduan.php">Panduan</a>
              <a class="btn btn-circle btn-success" href="rekap.php">Rekap Pengusulan</a>
            </div>
            <div class="clearfix"></div>
            <div class="col" style="margin-top: 30px;">
              <form class="form-signin" action="" method="post">
                <h3 class="text-center title-login" style="margin-bottom: 10px; color: #0c0b26;"><b>Register</b></h3>
                <hr>
                <?php
                if (isset($_GET["msg"]) && $_GET["msg"] == 'failed') {
                  echo '<div class="alert alert-danger alert-dismissable">NIP tidak terdaftar</div>';
                }else if (isset($_GET["msg"]) && $_GET["msg"] == 'gagal') {
                  echo '<div class="alert alert-danger alert-dismissable">NIP sudah ada akun.</div>';
                }
                ?>
                <div class="input-group">
                  <span class="input-group-addon"><i class="glyphicon glyphicon-user"></i></span>
                  <input id="nip" type="text" class="form-control" name="nip" value="" placeholder="NIP" oninput="cekNIP()" required>                         
                </div>
                <h5 style="color: red;" id="cek"></h5><br>
              ...
              <div class="form-group">
                <input type="submit" name="add" class="btn btn-primary btn-login-submit btn-block m-t-md" value="Register" />
              </div>
          ...
          <div class="form-group">
            <p class="text-center m-t-xs text-sm">Sudah memiliki akun?</p> 
            <a href="login.php" class="btn btn-primary btn-block m-t-md">Log In</a>
          </div>
